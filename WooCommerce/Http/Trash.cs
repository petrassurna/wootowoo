...
namespace WooCommerce.Http
{
  public class Trash
  {
    public static async Task EmptyWooCommerceTrashAsync(WordPressInstallation installation)
    {
      ...
      while (morePages)
      {
        ...
        {
          Console.WriteLine($"Deleting trashed product: {product.name} (ID: {product.id})");

          var deleteResponse = await httpClient.DeleteAsync($"{baseUrl}/{product.id}?force=true");
          if (deleteResponse.IsSuccessStatusCode)
          {
            Console.WriteLine($"Permanently deleted product ID {product.id}");
          }
          else
          {
            var error = await deleteResponse.Content.ReadAsStringAsync();
            Console.WriteLine($"Failed to delete product ID {product.id}: {error}");
          }
        }
      ...
      Console.WriteLine("All trashed products permanently deleted.");
    }
