...
namespace WooCommerce.Http.SourceInstallation.Categories
{
  public class CategoryFetcher : IFetcher
  {
    ...
    private readonly CategoryPusherHttp _categoryPusherHttp;
    private readonly CategoryFetcherHttp _categoryFetcherHttp;
    ...
    public CategoryFetcher(HttpClient httpClient, WordPressInstallation installation, ILogger logger, int maxConcurrency = 3, int requestDelayMs = 100)
    {
      ...
      _logger = logger;
      _categoryPusherHttp = new CategoryPusherHttp(httpClient, _installation, logger, maxConcurrency, requestDelayMs);

      _categoryFetcherHttp = new CategoryFetcherHttp(_httpClient, _installation, logger);
    }
    ...
    public async Task Fetch()
    {
      ...
      int countAtDestination = await _categoryFetcherHttp.GetCategoryCount();
